"""Run 10 — Combined best: RoPE + BPE + QK-Norm + no attn bias.
QK-Norm: RMSNorm on Q and K before attention, stabilizes training.
No attention bias: saves 3072 params (minor but free).
"""
import math
import torch
...
class Config:
    ...
    n_head = 6
    n_embd = 192
    dropout = 0.05        # Light regularization (from Run 8)
    tie_weights = True
    use_rope = True
...
class SelfAttention(nn.Module):
    def __init__(self, cfg):
        ...
        # No bias in QKV and proj — saves 4*n_embd params per block
        self.qkv = nn.Linear(cfg.n_embd, 3 * cfg.n_embd, bias=False)
        self.proj = nn.Linear(cfg.n_embd, cfg.n_embd, bias=False)
        self.drop = nn.Dropout(cfg.dropout)
        self.proj._is_residual = True
        # QK-Norm: RMSNorm on Q and K per head (stabilizes attention)
    ...
    def forward(self, x):
        ...
        # QK-Norm before RoPE (normalize per head)
        q = self.q_norm(q)
        k = self.k_norm(k)
        if self.use_rope:
