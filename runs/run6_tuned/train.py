"""Universal trainer for BPE runs. Loads pre-encoded .pt token files for instant startup."""
import argparse, math, time, os, torch, torch.nn.functional as F
...
def main():
    ...
    ap.add_argument("--data", help="Raw text file (slow — encodes with BPE)")
    ap.add_argument("--token_ids", help="Pre-encoded .pt file (fast — skip encoding)")
    ...
    if args.token_ids:
        # Fast path: load pre-encoded token IDs
        ids = torch.load(args.token_ids, map_location="cpu", weights_only=True)
        print(f"loaded pre-encoded: {len(ids):,} tokens (vocab {tok.vocab_size})")
    else:
        # Slow path: encode from raw text
